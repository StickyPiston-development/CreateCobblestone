...
public class GeneratorTypeLoader {
  ...
  public static void loadGeneratorTypes(ResourceManager resourceManager) {
    ...
    Map<ResourceLocation, Resource> resources = resourceManager.listResources("generator_types",
    ...
    for (Map.Entry<ResourceLocation, Resource> entry : resources.entrySet()) {
      ...
      try (InputStream inputStream = resource.open()) {
        ...
        if (generatorJsonData.has("enabled") && !generatorJsonData.get("enabled").getAsBoolean()) {
          // Enabled at default, but disabled when actively disabled. Completely removes
          // the generator type, also from creative tab.
          continue;
        }
        ...
        if (generatorJsonData.has("ratio")) {
          ...
          if (generatorJsonData.has("outputPerSecondPerRpm")) {
            LOGGER.error(
                "Generator type {} has both ratio and outputPerSecondPerRpm, outputPerSecondPerRpm will be used", id);
          } else {
            // Convert ratio to outputPerSecondPerRpm
            LOGGER.warn(
                "Generator type {} has deprecated ratio, please use outputPerSecondPerRpm instead. (Converted to {} outputPerSecondPerRpm)",
                id, 1 / (generatorJsonData.get("ratio").getAsFloat()) * 20);
        ...
        GeneratorType.initializeNewType(id.toString(), new ResourceLocation(block), generatorStress,
            outputPerSecondPerRpm, generatorStorage);
  ...
  public static void loadGeneratorTypesFromPacket(FriendlyByteBuf buf, NetworkManager.PacketContext context) {
    ...
    for (int i = 0; i < generatorAmount; i++) {
      ...
      GeneratorType.initializeNewType(id, new ResourceLocation(block), generatorStress, generatorRatio,
          generatorStorage);
    }
